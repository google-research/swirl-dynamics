...
class StochasticInterpolantModel(models.BaseModel):
  ...
  Attributes:
    ...
    noising_process_intp: A Callable that samples noise from the noising process
      for the interpolant.
  ...
  # loss function.
  noising_process_intp: Callable[[Array, ArrayShape], Array] = jax.random.normal
  ...
  def loss_fn(
    ...
    noise_interp = self.noising_process_intp(noise_rng, batch["x_0"].shape)
  ...
  def eval_fn(
    ...
    noise = self.noising_process_intp(noise_rng, x_1.shape)
...
class StochasticInterpolantCRPSModel(models.BaseModel):
  ...
  def loss_fn(
    ...
    # Interpolation between x_0 and x_1 (check the interpolant)
    # Check the dimensions here.
    noise_interp_1 = self.noising_process_intp(noise_rng, batch["x_0"].shape)
    noise_interp_2 = self.noising_process_intp(noise_rng_2, batch["x_1"].shape)
    if self.use_same_noise_rng:
      noise_flow_rng = noise_rng
    noise_flow = self.noising_process_flow(noise_flow_rng, batch["x_0"].shape)
