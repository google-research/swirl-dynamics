...
class PlotSamples(templates.MatplotlibFigureAsImage):
  ...
  def on_eval_batches_end(
    ...
    # All of `samples`, are expected to have shape (batch, lat, lon, dims).
    if isinstance(plot_data, dict):
      for num_steps, samples in plot_data.items():
        for idx_batch in range(
            0, samples.shape[0], self.plot_every_n_samples
        ):
          figures[f"num_steps_{num_steps}_sample_{idx_batch}"] = (
              self._make_imshow_plot(
