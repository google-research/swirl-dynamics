...
"""Trainers for Mean Flow models.

This scripts defines the trainers for the mean flow models, which
are defined following the templates in `swirl_dynamics.templates.trainers`.

This script also defines the train state for the mean flow models,
...
import jax
import optax
from swirl_dynamics.projects.distillation.mean_flow import models
...
class MeanFlowTrainState(train_states.BasicTrainState):
...
TrainState: TypeAlias = MeanFlowTrainState


class MeanFlowTrainer(trainers.BasicTrainer[models.MeanFlowModel, TrainState]):
  ...
  def inference_fn_from_state_dict(
    ...
    return models.MeanFlowModel.inference_fn(variables, *args, **kwargs)


class DistributedMeanFlowTrainer(
    MeanFlowTrainer,
    trainers.BasicDistributedTrainer[models.MeanFlowModel, TrainState],
):
  """Multi-device trainer for the mean flow models."""
