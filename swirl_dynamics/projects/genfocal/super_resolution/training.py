...
class DenoisingModel(templates.BaseModel):
  ...
  This class also implements various evaluation metrics that help monitor
  progress during training. Besides denoising RMSE at different noise levels,
  additional eval metrics include CRPS of the samples generated by a sampling
  SDE and eval data log likelihood computed from the probability flow ODE.
  Some generated samples are also returned from evaluation for visualization.
